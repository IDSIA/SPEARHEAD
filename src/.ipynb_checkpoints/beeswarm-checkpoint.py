...
COPY AND PASTE THIS FILE INSIDE THE _beeswarm.py FILE IN YOUR INSTALLATION OF SHAP. On macos, if you use conda, it shoud be under /Users/<yourname>/miniforge3/envs/fplier/lib/python3.10/site-packages/shap/plots/_beeswarm.py. Otherwise you must find where is shap package installed and then the path is the same (<your shap installation folder>/shap/plots/_beeswarm.py)
"""
...
def summary_legacy(
    ...
    if plot_type == "dot":
        for pos, i in enumerate(feature_order):
            ...
            if features is not None and colored_feature:
                ...
                if not plot_feature_names:
                    curr_axis.set_yticklabels(["" for _ in range(features.shape[1])])
            else:
                curr_axis.scatter(
                    shaps,
                    pos + ys,
        ...
        # if curr_axis is None:
        #     curr_axis = pl.gca()
        curr_axis.set_yticklabels([feature_names[i] for i in feature_inds], fontsize=13)
