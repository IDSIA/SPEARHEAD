...
def overview_calib(dt_preds: pd.DataFrame,
    ...
    fig_h = nrows_targets * (plot_size + plot_size * 0.3) + 1.5
    ...
    for i, tar in enumerate(targets):
        ...
        # Make calibration plot square
        # ax_calib.set_aspect('equal', adjustable='box')
...
def shap_helper(targets, data_folder, suffix=""):
    ...
    for target in targets:
        ...
        for rep in range(10):
            shap_values = np.load(
                f"{data_folder}/for_shap/{model_to_train}/shap_values_combined_{this_target_name}_repetition{rep}{'' if not is_ukbb else '_UKBB'}.npy"
            )
            shap_values_list.append(shap_values)
            
            X_all = pd.read_csv(
                f"{data_folder}/for_shap/{model_to_train}/X_all_{this_target_name}_repetition{rep}{'' if not is_ukbb else '_UKBB'}.csv", index_col=0
            ).reset_index(drop=True)
        ...
        summary_plot_mod(
            ...
            X_all.drop(columns="patient_id_hashed"),
            curr_axis=ax_dot,
...
if __name__ == "__main__":
    ...
    dt_preds = pd.concat([pd.read_csv(f"Data/predictions/dt_preds_RF_repetition{i}.csv", index_col=0) for i in range (10)], ignore_index=True).reset_index(drop=True)
    
    overview(
        dt_preds,
        targets=targets,
        ncols=3,
        filename="figures/compact_overview_RF.png",
    ...
    overview_calib(
        ...
        targets=targets,
        ncols=3,
        filename="figures/compact_overview_calib_RF.png",
        calib_bins=10,           # tweak as you like
        calib_strategy="quantile"  # "uniform" also OK
    )
