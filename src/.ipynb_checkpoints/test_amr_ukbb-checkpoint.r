...
# Read your data
df <- read.csv("src/src_data/all_UKBB_data.csv")
...
df <- df %>%
  mutate(gram_binary = case_when(
    ...
    TRUE ~ NA_real_  # for unknown or unclassified
  ))
...
sir_to_binary <- function(x) {
  case_when(
    x == "R" ~ 1,
    x %in% c("S", "I", "NI", "SDD") ~ 0,
    TRUE ~ NA_real_  # keep NA for unknown/missing values
  )
}
...
ggsave("figures/top_microorganisms_ukbb.png", width = 8, height = 8, dpi = 300)
...
cat("Processed data shape:", length(unique(data_clean_binary$patnr)), "\n")
