...
MODIFIED TO ALLOW FOR CUSTOM AXIS CHOICE WHEN PLOTTING. 
"""
...
def summary_legacy(
    ...
    if curr_axis is None:
        curr_axis = pl.gca()
    curr_axis.axvline(x=0, color="#999999", zorder=-1)

    if plot_type == "dot":
        for pos, i in enumerate(feature_order):
            curr_axis.axhline(y=pos, color="#cccccc", lw=0.5, dashes=(1, 5), zorder=-1)
            shaps = shap_values[:, i]
        ...
        if features is not None:
            ...
            for pos, i in enumerate(feature_order):
                ...
                xs = np.linspace(
                    np.min(shaps) - rng * 0.2, np.max(shaps) + rng * 0.2, 100
                )
                if np.std(shaps) < (global_high - global_low) / 100:
                    ds = gaussian_kde(
                        shaps
                        + np.random.randn(len(shaps)) * (global_high - global_low) / 100
                    )(xs)
                else:
        ...
        for pos, ind in enumerate(feature_order):
            ...
            for i in range(nbins):
                ...
                ys[i, :] = gaussian_kde(
                    shaps + np.random.normal(loc=0, scale=0.001, size=shaps.shape[0])
                )(x_points)
            ...
            for i in range(nbins - 1, -1, -1):
                y = ys[i, :] / scale
                c = (
                    pl.get_cmap(color)(i / (nbins - 1))
                    if color in pl.cm.datad
                    else color
        ...
        feature_inds = feature_order[:max_display]  # [::-1]
        y_pos = np.arange(len(feature_inds))
        global_shap_values = np.abs(shap_values).mean(0)

        # curr_axis.barh(y_pos, global_shap_values[feature_inds], 0.7, align='center', color=color)
        if errorbar_sd is not None:
            curr_axis = sns.barplot(
                x=global_shap_values[feature_inds],
                y=y_pos,
                xerr=errorbar_sd[feature_inds],
                width=0.6,
                orient="h",
                color=color,
                ax=curr_axis,
        ...
        curr_axis.set_yticks(y_pos)
        # if curr_axis is None:
        #     curr_axis = pl.gca()
        if plot_feature_names:
            curr_axis.set_yticklabels(
                [feature_names[i] for i in feature_inds], fontsize=15
            )
        ...
        # if curr_axis is None:
        #     curr_axis = pl.gca()
        curr_axis.set_yticklabels([feature_names[i] for i in feature_inds], fontsize=15)
        curr_axis.legend()
    ...
    if (
        ...
        m.set_array([0, 1])
        cb = pl.colorbar(m, ax=curr_axis, ticks=[0, 1], aspect=80)
        cb.set_ticklabels([labels["FEATURE_VALUE_LOW"], labels["FEATURE_VALUE_HIGH"]])
        cb.set_label(color_bar_label, size=12, labelpad=0)
        cb.ax.tick_params(labelsize=11, length=0)
        cb.set_alpha(1)
    ...
    curr_axis.xaxis.set_ticks_position("bottom")
    curr_axis.yaxis.set_ticks_position("none")
    curr_axis.spines["right"].set_visible(False)
    curr_axis.spines["top"].set_visible(False)
    curr_axis.spines["left"].set_visible(False)
    curr_axis.tick_params(color=axis_color, labelcolor=axis_color)
    # if plot_feature_names: # original
    #     pl.yticks(
    #         range(len(feature_order)),
    ...
    #     )
    if not plot_feature_names:
        curr_axis.set_yticks(range(len(feature_order)), [])
        curr_axis.tick_params(axis="y", which="both", length=0)
    if plot_type != "bar":
        curr_axis.tick_params("y", length=20, width=0.5, which="major")
    curr_axis.tick_params("x", labelsize=11)
    curr_axis.set_ylim(-1, len(feature_order))
    if plot_type == "bar":
        curr_axis.set_xlabel(labels["GLOBAL_VALUE"], fontsize=15)
    else:
        curr_axis.set_xlabel(labels["VALUE"], fontsize=15)
    # pl.tight_layout()
    if show:
        pl.show()
